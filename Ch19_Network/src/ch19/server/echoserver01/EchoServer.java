...
public class EchoServer {
	public static void main(String[] args) {
		...
		try {
			...
			// 새로운 소켓이 생성되서 클라이언트와 연결된 소켓이 된다.
			// 클라이언트와 직접 통신(서비스)하는 소켓 객체 리턴
			// 클라이언트와 연결과정(스트림 형성 과정)이 완료되면 리턴
			Socket socket = server.accept(); // accept는 서버와 연결시켜주는 역할을 한다.
			 ...
			 * */
			OutputStream out = socket.getOutputStream();	// Client한테 전송
			InputStream in  = socket.getInputStream();		// from Client로부터 수신
			...
			while(true) {				
				...
				pw.println(line);	// 데이터 + '\n'		개행문자를 써주면서 프로그램의 끝을 알려준다.
				pw.flush();			// 버퍼에 남아있는 데이터 즉시 전송해라
			}
