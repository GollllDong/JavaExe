...
public class SimpleWebServer {
	...
	public static void main(String[] args) {
		final int PORT = 80; // http는 디폴트로 80번 포트를 사용한다.
		try {
			// 현재 서버 프로세스가 동작하는 환경의 ip 주소를 자동으로 할동한다.
			...
			while (true) {
				...
				HttpThread ht1 = new HttpThread(socket);
				ht1.start();
			}
...
class HttpThread extends Thread {
	...
	public void run() {
		try {
			...
			System.out.printf("<서버-%s>%s", getName(), e.getMessage());
		}
	}
}